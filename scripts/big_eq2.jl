...
r0 = get_interpolation_over_u(uc0, r0c, u)
z0 = get_interpolation_over_u(uc0, z0c, u)
p0 = get_interpolation_over_u(uc0, p0c, u)
h0 = get_interpolation_over_u(uc0, h0c, u)
sig0 = get_interpolation_over_u(uc0, sig0c, u)
...
dy[12].= 0#zeros(size(p));

# @infiltrate

end

function get_interpolation_over_u(x_vals, y_vals, u)
  y_val_interp = interpolate((x_vals,), y_vals, Gridded(Linear()))
  return y_val_interp.(u) 

end


function shapefsingle!(dy, y, params, u)
...
function guessfsingle(u,ka,R0,ri,sigeq)
  # psi = y(1);
  # dpsi = y(2);
  # r = y(3);
  # z = y(4);
  ...
  g = [0,
       ...
       sigeq,
       0.0,
       2/3*R0^3*2, #(2 .+ cos.(u*pi)).*sin.(u*pi/2).^4,
       R0*pi]
  return stack(g)
end
...
#   h = para(1);
  ...
  @assert allequal(h)
  @assert allequal(y[begin][:,2])
  # @infiltrate
  alpha_i = ka/2*ri*(c0^2-y[begin][:,2][1]l^2/h[1].^2);
  
  if Ga2 == 0 # No tangential friction
    #   beta_z_bc0 = ya(6) .+ yb(6); # beta(0) .+ beta(1) = 0
    #   beta_z_bc1 = ya(4) .+ yb(4); # z(0) .+ z(1) = 0
    # beta_z_bc0 = y[:,6][begin] .+ y[:,6][end] #removed this
    # beta_z_bc1 = y[:,4][begin] .+ y[:,4][end] #andthis

    beta_z_bc0 = y[begin][:,6] .+ y[end][:,6] #removed this
    beta_z_bc1 = y[begin][:,4] .+ y[end][:,4]
  else
    #   beta_z_bc0 = ya(6); # beta(1) = 0
    #   beta_z_bc1 = yb(6); # beta(1) = 0
    beta_z_bc0 = y[begin][:,6]
    beta_z_bc1 = y[end][:,6]
  end

  # @infiltrate

  res = [ y[begin][:,1][1]-0;   # psi(0) = 0;
          y[begin][:,3]-ri;      # r(0) = ri;
          beta_z_bc0;       
          y[begin][:,5]-alpha_i; # alpha(0)=ka/2*(c0^2-dpsi^2/h^2)...
          beta_z_bc1;    # 
          y[begin][:,7]-0;       # vu(0) = 0;
          y[begin][:,10]-0;      # sigma'(0)=0;
          y[begin][:,11]-0;      # vol(0) = 0;
          y[begin][:,12]-h[1];      # h(0) = h0;
          y[end][:,1]-pi;      # psi(1) = pi;
          y[end][:,3]-ri;      # r(1) = ri;
          y[end][:,7]-0;       # vu(1) = 0;
          y[end][:,10]-0;      # sigma'(1) = 0;
          y[end][:,11]-vol0];     # vol(1) = vol0;
  # res[1] = y[:,1][begin]-0;   # psi(0) = 0;
  # res[2]=        y[:,3][begin]-ri;      # r(0) = ri;
  # res[3]=        beta_z_bc0;       
  #   res[4]=      y[:,5][begin]-alpha_i; # alpha(0)=ka/2*(c0^2-dpsi^2/h^2)...
  #     res[5]=    beta_z_bc1;    # 
  #       res[6]=  y[:,7][begin]-0;       # vu(0) = 0;
  #      res[7]=   y[:,10][begin]-0;      # sigma'(0)=0;
  #      res[8]=   y[:,11][begin]-0;      # vol(0) = 0;
  #      res[9]=   y[:,12][begin]-h[1];      # h(0) = h0;
...
function twobcfsingle!(res, y, p, x)
  # function bc1!(res, y, p)
    # psi = y(1);
    # dpsi = y(2);
    # r = y(3);
  ...
  #   h = para(1);
    ...
    # @assert allequal(y[begin][:,2])
    @infiltrate
    alpha_i = ka/2*ri*(c0^2-y[begin][2]^2/h.^2);
...
#   h = para(1);
  ...
  #   h = para(1);
    ...
    #         y[:,10][end]-0;      # sigma'(1) = 0;
    #         y[:,11][end]-vol0];     # vol(1) = vol0;
    res[1] = y[:,1][end]-pi;      # psi(1) = pi;
    res[2]=      y[:,3][end]-ri;      # r(1) = ri;
    res[3]=      y[:,7][end]-0;       # vu(1) = 0;
    res[4]=      y[:,10][end]-0;      # sigma'(1) = 0;
    res[5]=      y[:,11][end]-vol0;
